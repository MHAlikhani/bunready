...
- Phase 6 (release pipeline): **implemented** - `release.yml`, SBOM, checksums and
  `docs/RELEASING.md`. The `v0.1.0` tag is blocked on O10 (npm trusted publishing).
- Phase 4 (`--run`): **not started** and the last functional gap.
...
1. Configure npm trusted publishing (O10), then tag `v0.1.0` and let
   `release.yml` do the rest.
2. Phase 4 (`--run`) remains the last functional gap.
3. Triage the Dependabot PRs (O9).
